    ...
    function showBetStatistData(arrBetData){
        ...
        if(arrBetData != null && arrBetData.length > 0){
            for(var idx in arrBetData){
                ...
                tHtml += " ondblclick=\"showEmpBetDlg('"+arrBetData[idx].mb_uid+"');\" >";
                tHtml += "<td><div class=\"cell\">";
                tHtml += arrBetData[idx].mb_uid;
                tHtml += "</div></td>";
                tHtml += "<td><div class=\"cell\">";
    ...
    function showBetDlgData(arrBetData){
        ...
        if(tHtml.length < 1){
            ...
            $("#el-dialog-footer-id").hide();
        } else {

            $("#el-dialog__empty-id").hide();
            $("#el-dialog-footer-id").show();

            $("#el-dialog-sum_1-id").text(nBetSum.toLocaleString());
            $("#el-dialog-sum_2-id").text(nWinSum.toLocaleString());
            $("#el-dialog-sum_3-id").text((nBetSum-nWinSum).toLocaleString());
        }
    ...
    function requestBetStatist(){
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {json_: jsonData},
            url:"/capi/betcompanystatist"+location.search,
    ...
    function requestBetHistory(){
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {json_: jsonData},
            url:"/capi/betagencystatist"+location.search,
            success: function(jResult) {
                //console.log(jResult);
                if(jResult.status == "success")
                {
